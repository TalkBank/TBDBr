...
## A glimpse at each talkbank query function
...
tokens <- getTokens(corpusName = 'childes', corpora = c('childes', 'Eng-NA', 'MacWhinney', '010411a'));
```
...
token.types <- getTokenTypes(corpusName = 'childes', corpora = c('childes', 'Eng-NA', 'MacWhinney'));
```
...
```{r utterances}
utterances <- getUtterances(corpusName = 'childes', corpora = c('childes', 'Eng-NA', 'MacWhinney', '010411a'))
```

```{r utterance output, echo = F}
kable(utterances[1:5,]) %>% 
   kable_styling("striped") %>%
   scroll_box(width = "100%")
```
...
cql.myball <- getCQL(cqlArr=list(list(type="lemma", item="my", freq="once"), list(type="lemma", item="ball", freq="once")), corpusName = 'childes', corpora = c('childes', 'Eng-NA', 'MacWhinney'));
```
